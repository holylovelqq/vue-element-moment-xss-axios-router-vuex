...
<template>
  <div>
    ...
    <div class="kbdetail" v-show="!isLoading">
      <div class="title">
      ...
      <div class="right">
        ...
        <p>{{formatTime(KBDetail.create)}}
          <!-- &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{{KBDetail.auther}} -->
        </p>
        <p class="right__edit">最終更新</p>
        <p>{{formatTime(KBDetail.edit)}}
          <!-- &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{{KBDetail.editer}} -->
        </p>
      </div>
      <!-- comment content -->
...
import RichText from '@/basics/RichText'
...
export default {
  ...
  methods: {
    // 編集ページに遷移
    goEditPage () {
      this.$router.push('/knowledge/' + this.KBDetailId + '/edit')
    },
    ...
    updateComment (id, content) {
      ...
      this.$axios.patch(path, { content: content.trim() })
        .then((res) => {
          if (res.status === 200) {
            this.getComments()
  ...
  computed: {
    isLoading: function () {
      let detailLength = Object.keys(this.KBDetail).length
      if (detailLength !== 0 && this.commentsGetted) {
        return false
      } else {
        return true
      }
